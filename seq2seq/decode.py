...
def beam_search_decode(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
                       tgt_tokenizer: spm.SentencePieceProcessor, args, device: torch.device, beam_size: int = 5, alpha: float = 0.7, stopping_criterion=None, threshold: float = 0.0):
    ...
    for _ in range(max_out_len):
        new_beams = []
        best_score = max(s for _, s in beams)
...
# Given a pruning threshold rp and an active candidate list C, a candidate cand ∈ C is discarded if:score(cand) ≤ rp ∗ max{score(c)}

def beam_search_decode(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
                       tgt_tokenizer: spm.SentencePieceProcessor, args, device: torch.device, beam_size: int = 5, alpha: float = 0.7, rp: float = 0.0):
    """Beam Search decoding compatible with Transformer-based Seq2Seq models."""
    model.eval()
    BOS, EOS, PAD = tgt_tokenizer.bos_id(), tgt_tokenizer.eos_id(), tgt_tokenizer.pad_id()
    encoder_out = model.encoder(src_tokens, src_pad_mask)
    ...
    for _ in range(max_out_len):
        new_beams = []
        for seq, score in beams:
            if score <= rp * max([s for _, s in beams]):
                continue
            if seq[0, -1].item() == EOS:
                new_beams.append((seq, score))
                continue
